"""
Command Switching with Motor Command Delay
Test sensitivity to actuator command latency (output-side delay)
Distinct from observation delay (input-side delay) tested in Stage 3
"""
import time
...
if __name__ == "__main__":
    ...
    parser.add_argument("--scenario", type=str, default="S2_turn", choices=SCENARIOS.keys())
    parser.add_argument("--motor_delay", type=int, default=0, help="Motor command delay in policy steps (0, 1, 2)")
    parser.add_argument("--switch_time", type=float, default=3.0)
    parser.add_argument("--duration", type=float, default=6.0)
    parser.add_argument("--log_dir", type=str, default="logs/sim2sim/motor_delay")
    ...
    motor_delay = args.motor_delay
    
    print(f"=== Command Switching: {scenario['name']} ===")
    print(f"Motor command delay: {motor_delay} steps ({motor_delay * 20} ms)")
    ...
    log_file = os.path.join(log_dir, f"mujoco_{args.scenario}_motordelay{motor_delay}_{timestamp}.npz")
    ...
    # Motor command delay buffer - stores target_dof_pos
    # tau is computed from target_dof_pos each physics step, so we delay target_dof_pos
    target_buffer = deque(maxlen=motor_delay + 1) if motor_delay > 0 else None
    delayed_target_dof_pos = default_angles.copy()

    def run_step():
        global action, target_dof_pos, delayed_target_dof_pos, obs, counter, fallen, fall_time, sim_time
        ...
        # Use delayed target for PD control (motor command delay)
        tau = pd_control(delayed_target_dof_pos, d.qpos[7:], kps, np.zeros_like(kds), d.qvel[6:], kds)
        d.ctrl[0:3] = tau[3:6]
        d.ctrl[3:6] = tau[0:3]
        d.ctrl[6:9] = tau[9:12]
        d.ctrl[9:12] = tau[6:9]
        ...
        if counter % control_decimation == 0:
            ...
            if not fallen and (abs(roll) > 1.0 or abs(pitch) > 1.0 or d.qpos[2] < 0.15):
                fallen = True
                fall_time = sim_time
                print(f"FALLEN at t={sim_time:.3f}s (roll={np.degrees(roll):.1f}°, pitch={np.degrees(pitch):.1f}°)")
            ...
            # Apply motor delay: buffer the new target, use delayed one
            if motor_delay > 0:
                target_buffer.append(target_dof_pos.copy())
                if len(target_buffer) > motor_delay:
                    delayed_target_dof_pos = target_buffer[0]
                else:
                    delayed_target_dof_pos = default_angles.copy()  # Use default until buffer fills
            else:
                delayed_target_dof_pos = target_dof_pos
    ...
    print(f"\n=== {scenario['name']} Metrics (motor_delay={motor_delay} steps, {motor_delay*20}ms) ===")
