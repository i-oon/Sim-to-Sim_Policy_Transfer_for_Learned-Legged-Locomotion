"""
Command Switching with Observation Delay
Test sensitivity to real-time sensing latency
"""
import time
...
if __name__ == "__main__":
    ...
    parser.add_argument("--delay", type=int, default=0, help="Observation delay in policy steps (0, 1, 2)")
    parser.add_argument("--switch_time", type=float, default=3.0)
    parser.add_argument("--duration", type=float, default=6.0)
    parser.add_argument("--log_dir", type=str, default="logs/sim2sim/delay")
    ...
    log_file = os.path.join(log_dir, f"mujoco_{args.scenario}_delay{obs_delay}_{timestamp}.npz")
    ...
    obs_buffer = deque(maxlen=obs_delay + 1) if obs_delay > 0 else None
    
    def build_obs(qj, dqj, quat, cmd):
        ...
        obs_now = np.zeros(num_obs, dtype=np.float32)
        obs_now[0:3] = base_lin_vel * lin_vel_scale
        obs_now[3:6] = base_ang_vel * ang_vel_scale
        obs_now[6:9] = gravity_orientation
        obs_now[9:12] = cmd * cmd_scale
        obs_now[12:24] = (qj - default_angles) * dof_pos_scale
        obs_now[24:36] = dqj * dof_vel_scale
        obs_now[36:48] = action
        return obs_now, base_lin_vel, base_ang_vel
    ...
    def run_step():
        ...
        if counter % control_decimation == 0:
            ...
            obs_now, base_lin_vel, base_ang_vel = build_obs(qj, dqj, quat, cmd)
            
            # Apply delay: use obs from (delay) steps ago
            if obs_delay > 0:
                obs_buffer.append(obs_now.copy())
                if len(obs_buffer) > obs_delay:
                    obs = obs_buffer[0]  # Use delayed observation
                else:
                    obs = obs_now  # Not enough history yet, use current
            else:
                obs = obs_now
    ...
    print(f"\n=== {scenario['name']} Metrics (delay={obs_delay} steps) ===")
